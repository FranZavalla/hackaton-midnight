...
class Contract {
  ...
  constructor(...args) {
    ...
    this.circuits = {
      ...
      close_ballot: (...args_0) => {
        if (args_0.length !== 1)
          throw new __compactRuntime.CompactError(`close_ballot: expected 1 argument (as invoked from Typescript), received ${args_0.length}`);
        const contextOrig = args_0[0];
        if (!(typeof(contextOrig) === 'object' && contextOrig.originalState != undefined && contextOrig.transactionContext != undefined))
          __compactRuntime.type_error('close_ballot',
                                      'argument 1 (as invoked from Typescript)',
                                      'ballot.compact line 37, char 1',
                                      'CircuitContext',
                                      contextOrig)
        const context = { ...contextOrig };
        const partialProofData = {
        ...
        const result = this.#_close_ballot_0(context, partialProofData);
        partialProofData.output = { value: [], alignment: [] };
        return { result: result, context: context, proofData: partialProofData };
      },
      add_organizer: (...args_0) => {
        if (args_0.length !== 2)
          throw new __compactRuntime.CompactError(`add_organizer: expected 2 arguments (as invoked from Typescript), received ${args_0.length}`);
        const contextOrig = args_0[0];
        const pk = args_0[1];
        if (!(typeof(contextOrig) === 'object' && contextOrig.originalState != undefined && contextOrig.transactionContext != undefined))
          __compactRuntime.type_error('add_organizer',
                                      'argument 1 (as invoked from Typescript)',
        ...
        const result = this.#_add_organizer_0(context, partialProofData, pk);
        partialProofData.output = { value: [], alignment: [] };
        return { result: result, context: context, proofData: partialProofData };
      }
    };
    this.impureCircuits = {
      ...
      close_ballot: this.circuits.close_ballot,
      add_organizer: this.circuits.add_organizer
    };
  }
  initialState(...args) {
    if (args.length !== 2)
    ...
    state.setOperation('close_ballot', new __compactRuntime.ContractOperation());
    state.setOperation('add_organizer', new __compactRuntime.ContractOperation());
    const context = {
      originalState: state,
  ...
  #_close_ballot_0(context, partialProofData) {
    let tmp;
    __compactRuntime.assert((tmp = this.#_public_key_0(context,
                                                       partialProofData,
                                                       this.#_secret_key_0(context,
                            ...
                            'Only the organizer can close the ballot');
    Contract._query(context,
                    partialProofData,
                    [
                     { push: { storage: false,
  ...
  #_add_organizer_0(context, partialProofData, pk) {
    __compactRuntime.assert(this.#_equal_0(pk,
                                           this.#_public_key_0(context,
                                                               partialProofData,
                                                               this.#_secret_key_0(context,
                                                                                   partialProofData))),
                            'Only the organizer can add authorities');
    Contract._query(context,
                    partialProofData,
                    [
                     { idx: { cached: false,
